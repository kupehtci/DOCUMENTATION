...
* `useEffect` allows to execute side effects on each render on in a controlled way in the component. 
* `useContext` allows to manage the context, a global state of the application to share data between components. 
* `useRef`
* `useReducer`
* `useCallback`
* `useMemo`
